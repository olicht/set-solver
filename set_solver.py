...
def resize_image(img, new_width=1200):
	"""Given cv2 image object and maximum dimension, returns resized image such that height or width (whichever is
...
def pretty_print_properties(properties):
	for p in properties:
		...
		print '%d %s %s %s' % (num, sc.PROP_COLOR_MAP[color], sc.PROP_SHAPE_MAP[shape], sc.PROP_TEXTURE_MAP[texture])
...
def get_approx_poly(card, do_rectify=True):
	...
	# l = len(approximated_poly)
	# if l != 4:
	# for p in approximated_poly:
	# cv2.circle(image, (p[0][0], p[0][1]), 3, (255,0,0), 5)
	# util.show(image)
...
def find_contours(bin_img, num=-1):
	derp, contours, hierarchy = cv2.findContours(bin_img, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
...
def get_binary(img, thresh=150):
	...
	opened = cv2.morphologyEx(threshold, cv2.MORPH_OPEN, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (17, 17)))
	# closed = cv2.morphologyEx(threshold, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (4, 4)))
	return opened
...
def get_binary_from_hsv(card):
	...
	_, bin_val = cv2.threshold(np.array(val), thresh=140, type=cv2.THRESH_BINARY_INV, maxval=255)
...
def get_texture_from_hue(hue, contour_box):
	...
	hue_bg = np.mean([hue[0:20, 0:20], hue[card_h - 20:card_h, card_w - 20:card_w], hue[0:20, card_w - 20:card_w],
