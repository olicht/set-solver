...
class Game:
	...
	@staticmethod
	def is_set(x, y, z):
		"""Returns true if card trio is a set at every position based on sum mod 3, which is only 0 IFF all same or
		all different """
		return sum(sum(i) % 3 for i in zip(x, y, z)) == 0
	
	def compare_all(self, idx=False):
		"""Compare all combinations of three cards and return list of tuples of winning sets. If idx, also return
		corresponding list index of each card. """
		sets = []
		trios = itertools.combinations(enumerate(self.cards), 3)
		for x, y, z in trios:
