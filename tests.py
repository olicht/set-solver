...
def play_game(frame, printall=False, draw_contours=True, resize_contours=True, draw_rects=False, sets_or_no=False,
              pop_open=True):
    ...
    try:
        contours, drawn, detected = s.detect_cards(
            img, draw_rects=True, return_contours=True)
    ...
    if sets_or_no:
        ...
        BORDER_SCALAR = 0.01
        border_h, border_w = (int(dim * BORDER_SCALAR)
        ...
        img_outlined = cv2.copyMakeBorder(img, border_h, border_h, border_w, border_w, cv2.BORDER_CONSTANT,
